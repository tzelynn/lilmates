...
- **Assets** (`assets/mates/`): Each subfolder is a cat. Must contain `1.PNG`, `2.PNG`, `3.PNG` for frames. Accessories are `acs_<name>.PNG`. Discovered at activation by `src/assetDiscovery.ts`.
...
- Frame animation follows a ping-pong pattern: indices [0, 1, 2, 1] repeating. After 3 seconds idle, resets to frame 0.
